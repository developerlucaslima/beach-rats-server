...
interface AddPlayerModalityAndStatsUseCaseRequest {
  playerId: string
  modalityId: string
  skillsIdsWithCategory: { skillId: string; category: Category }[]
}

interface AddPlayerModalityAndStatsUseCaseResponse {
  success: boolean
}

export class AddPlayerModalityAndStatsUseCase {
  constructor(
  ...
  }: AddPlayerModalityAndStatsUseCaseRequest): Promise<AddPlayerModalityAndStatsUseCaseResponse> {
