...
export class PrismaPlayerModalityMonthsStatsRepository
  implements IPlayerModalityMonthsStatsRepository
{
  async findByPlayerModalityIdAndMonth(params: {
    playerModalityId: string
    month: Date
  }) {
    const { playerModalityId, month } = params
    const playerModalityMonthStats =
      await prisma.playerModalityMonthStats.findUnique({
        where: {
